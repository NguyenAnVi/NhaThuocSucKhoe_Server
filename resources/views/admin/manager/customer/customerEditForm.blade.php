<form id="edit-form" class="uk-grid-small uk-form uk-child-width-1-1" method="POST" action="{{ route('admin.customer.update', ['customer' => $user->id]) }}" uk-grid="">
	@csrf
	@method('PUT')
	<div class="uk-form" uk-grid>
		<div class="uk-width-expand uk-grid-match" uk-grid>
			...
			<div class="uk-width-1-1@s uk-width-3-4@l">
				<input value="@if(old('name')!=NULL){{old('name')}}@else{{$user->name}}@endif" tabindex="1" class="uk-input uk-form-large @error('name') uk-form-danger @enderror" type="text" name="name" placeholder="Họ và tên">
				@error('name')
	...
	<div class="uk-form" uk-grid>
		<div class="uk-width-expand uk-grid-match" uk-grid>
			...
			<div class="uk-width-1-1@s uk-width-3-4@l">
				<input value="@if(old('phone')!=NULL){{old('phone')}}@else{{$user->phone}}@endif" tabindex="1" class="uk-input uk-form-large @error('phone') uk-form-danger @enderror" type="text" name="phone" placeholder="Điện thoại">
				@error('phone')
				<span class="uk-text-danger">
					<strong>{{ $message }}</strong>
				</span>
