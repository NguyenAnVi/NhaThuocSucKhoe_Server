<div class="uk-overflow-auto">
	<table class="uk-table uk-table-middle  uk-table-hover uk-table-divider uk-table-small">
		...
		<tbody id="ajx" uk-scrollspy="cls: uk-animation-fade; target: tr; delay: 300;">
				@each('admin.manager.hr.hrTableRow', $collection, 'item')
		</tbody>
	</table>
</div>
		...
		let o = '';
			collection.forEach(function(item) {
				o += '<tr>';
					...
					o += '<td>' + item.name + '</td>';
					o += '<td>'+ item.phone + '</td>';
					o += '<td><button class="uk-button-primary uk-icon-button uk-form-small" type="submit" onclick="window.location.href=\'/admin/hr/'+item.id+'/edit\'"><span uk-icon="pencil"></span></button></td>';
					o += '<td><form id="item-'+item.id+'-destroy-form" method="POST" hidden action="/admin/hr/'+item.id+'"><input type="hidden" name="_token" value="{{csrf_token()}}"><input type="hidden" name="_method" value="delete"></form><button data-id="'+ item.id +'" class="confirm-delete uk-button-danger uk-icon-button uk-form-small" type="submit"><span uk-icon="close"></span></button></a></td>'
				o += '</tr>';
			});
		return o;
	}
